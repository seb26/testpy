...
* ''See also:'' [[Special:RecentChangesLinked/Portal Wiki:Reports/All_English_articles|Recent changes to English articles]]
...
bad_sub = re.compile('\/(ar|cs|da|de|es|fi|fr|hu|it|ja|ko|nl|no|pl|pt|pt-br|ro|ru|sv|zh-hans|zh-hant|OTFWH|titles|Archive|Header|Footer|diff)')

for j in res['query']['allpages']:
    if not bad_sub.search(j['title']):
        pageid = j['pageid']
        title = j['title']
        table_row = u'# [[%s]]' % title
